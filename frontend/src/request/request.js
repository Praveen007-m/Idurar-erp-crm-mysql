...
function appendFormData(formData, value, key) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => appendFormData(formData, item, `${key}[${index}]`));
    return;
  }

  if (typeof value === 'object') {
    Object.entries(value).forEach(([nestedKey, nestedValue]) => {
      appendFormData(formData, nestedValue, `${key}[${nestedKey}]`);
    });
    return;
  }
...
function toMultipartFormData(values = {}) {
  const formData = new FormData();
  Object.entries(values).forEach(([key, value]) => appendFormData(formData, value, key));
  return formData;
}
...
successHandler(response, {
  ...
  createAndUpload: async ({ entity, jsonData }) => {
    try {
      includeToken();
      const formData = jsonData instanceof FormData ? jsonData : toMultipartFormData(jsonData);
